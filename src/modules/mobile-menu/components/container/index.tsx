    ...
    <Transition.Root show={state} as={Fragment}>
      <Dialog as="div" className="fixed inset-0 flex z-50" onClose={close}>
        <Transition.Child
          ...
          leaveTo="opacity-0"
        >
          <Dialog.Overlay className="absolute inset-0 bg-muted bg-opacity-75 transition-opacity backdrop-blur-sm" />
        </Transition.Child>
        ...
        >
          <div className="absolute inset-0 overflow-hidden trans">
            <div className="pointer-events-none fixed inset-y-0 right-left flex max-w-full">
              <div className="relative w-screen pointer-events-auto bg-background flex flex-col overflow-y-auto">
                {children}
              </div>
            </div>
          </div>
